...
let seoSettings = {
  ...
  metaDescription:
    "Find the best flight deals, hotels, cruises, and vacation packages at unbeatable prices. Book your next adventure with travelofareinfo.com - Your trusted travel partner.",
  keywords:
    "flight deals, cheap flights, hotels, cruises, vacation packages, travel deals, airline tickets",
  ogImage: "https://travelofareinfo.com/og-image.jpg",
  canonicalUrl: "https://travelofareinfo.com",
};
...
let websiteContent = {
  ...
  heroSubtitle: '',
  aboutText: '',
  contactEmail: 'info@travelofareinfo.com',
...
let gscSettings = {
  ...
  siteUrl: process.env.GSC_SITE_URL || 'https://travelofareinfo.com',
  enabled: false,
  connected: false
};
...
router.get('/seo/audit', verifyAdminToken, async (req, res) => {
  try {
    const siteUrl = process.env.GSC_SITE_URL || 'https://travelofareinfo.com';
...
router.post('/blog/ai/generate', verifyAdminToken, async (req, res) => {
  try {
    ...
    const blogUrl = `https://travelofareinfo.com/blog/${aiPost.slug}`;
